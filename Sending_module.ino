...
#define BME280_ADRESS (0x76)                                                                          // BME280 I2C adress
#define ADS_1115_ADRESS (0x48)                                                                        // ADC I2C adress 
...
void setup()
{
  ...
  BME280.begin(BME280_ADRESS);                                                                        // Inicialization BME280 on I2C adress, next step is setting for parameters (selected setting for lowest power consumption)
...
//  int distance = Water_level();
  ...
  uint8_t batteryLevel = constrain(map(aku_volt, 3200, 4150, 0, 100), 0, 100);                        // Data format adjustment (Battery level betwen 4V (100%) and 3,2V (0%))
  ...
  int brightness = 250;                                                                               // Value from luxmeter, so far only as a static value, it will be completed upon delivery of the part    
